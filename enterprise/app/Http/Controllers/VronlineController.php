...
class VronlineController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $posCode       = [
            ...
            'index-video-rank'       => 5,
            'index-hardware-pic'     => 5,
            'index-hardware-new'     => 3,
            'index-hardware-news'    => 5,
            'index-hardware-rank'    => 5,
            'index-hardware-subject' => 1,
        ];
    ...
    public function pcSearch(Request $request, $words, $page = 1)
    {
        $vronlineModel = new VronlineModel;
        $posCode       = [
            'game-rank-vrgame-mostpv' => 10,
        ...
        $limit     = [self::PAGE_SIZE * ($page - 1), self::PAGE_SIZE];
        $solrModel = new SolrModel;
        $articles  = $solrModel->searchVronline(['title' => $words, 'tp' => 'pc', 'limit' => $limit], true, $articleNum);
        $pageObj   = new LengthAwarePaginator([], $articleNum, self::PAGE_SIZE, $page);
        $pageObj->appends(['base' => '/vronline/search/' . $words]);
        return view('vronline.search', ['tops' => $tops, 'articles' => $articles, 'pageObj' => $pageObj, 'words' => $words, 'total' => $articleNum]);
    }
    ...
    public function author(Request $request, $id, $page = 1)
    {
        ...
        $limit     = [self::PAGE_SIZE * ($page - 1), self::PAGE_SIZE];
        $solrModel = new SolrModel;
        $articles  = $solrModel->searchVronline(['author' => $id, 'limit' => $limit], true, $articleNum);
        $pageObj   = new LengthAwarePaginator([], $articleNum, self::PAGE_SIZE, $page);
        $pageObj->appends(['base' => '/vronline/author/' . $id]);
        return view('vronline.search', ['tops' => $tops, 'articles' => $articles, 'pageObj' => $pageObj, 'authorId' => $id]);
    }

    public function authorInfo(Request $request)
    {
        $vronlineModel = new VronlineModel;
        $authors       = $vronlineModel->getAuthors();
        $out           = [];
        if ($authors) {
            foreach ($authors as $author) {
                $out[$author['writer_id']] = $author;
            }
        }
    ...
    public function articleDetail(Request $request, $id)
    {
        ...
        $categoryName = $article['article_tp'];
        ...
        $category       = Library::getCurrentCategory($categoryName, $article['article_category']);

        $relatedArticles = $solrModel->searchVronline(['itemid' => $id, 'tp' => $categoryName, 'orderBy' => 'random_' . mt_rand(1000, 2000) . ' desc', 'category' => $category['id'], 'limit' => [0, 3]]);
        ...
        $categoryLink = $categoryName == "news" ? "article" : "pc";
        return view('vronline.article_detail', ['article_id' => $id, 'article' => $article, 'updateTime' => $updateTime, 'authorArticleNums' => $authorArticleNums, 'category' => $category, 'tops' => $tops, 'authorArticles' => $authorArticles, 'relatedArticles' => $relatedArticles, 'uid' => $uid, 'categoryLink' => $categoryLink]);
    }
    ...
    public function videoDetail(Request $request, $id)
    {
        ...
        $solrModel      = new SolrModel;
        $videoInfoAfter = $solrModel->searchVronline(['tp' => 'video', 'orderBy' => 'random_' . mt_rand(1000, 2000) . ' desc', 'limit' => [0, 5]]);
