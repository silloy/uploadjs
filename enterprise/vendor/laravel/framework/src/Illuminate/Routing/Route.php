...
class Route {
	...
	/**
	 * The HTTP get html cache.
	 *
	 * @var string
	 */
	protected $cacheTime;
	protected $chacheFile;
	/**
	 ...
	 * @return void
	 */
	public function __construct($methods, $uri, $action, $cacheTime) {
		$this->uri = $uri;
	...
	public function run(Request $request) {
		//cache html
		if (!getenv('LARAVEL_APP_ENV') && $this->cacheTime > 0 && in_array("GET", $this->methods)) {
			$staorge_path = storage_path();
			$fileName = md5($request->url());
			$pathPre = substr($fileName, 0, 2);
			$path = $staorge_path . '/static/' . $pathPre;
			if (!file_exists($path)) {
				mkdir($path);
			}
			$cacheFile = $path . "/" . $fileName;
			if (file_exists($cacheFile)) {
				$lastTime = fileatime($cacheFile);
				if (time() - $lastTime < $this->cacheTime) {
	...
	protected function runWithCustomDispatcher(Request $request) {
		...
		$response = $dispatcher->dispatch($this, $request, $class, $method);

		if ($this->cacheTime > 0 && $this->cacheFile) {
