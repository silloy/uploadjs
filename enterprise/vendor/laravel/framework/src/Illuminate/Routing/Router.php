...
class Router implements RegistrarContract {
	 ...
	 */
	public function get($uri, $action, $cacheTime = 0) {
		return $this->addRoute(['GET', 'HEAD'], $uri, $action, $cacheTime);
	}
	 ...
	 * @return \Illuminate\Routing\Route
	 */
	public function post($uri, $action, $cacheTime = 0) {
	 ...
	 * @return \Illuminate\Routing\Route
	 */
	protected function addRoute($methods, $uri, $action, $cacheTime = 0) {
		return $this->routes->add($this->createRoute($methods, $uri, $action, $cacheTime));
	}
	 ...
	 * @return \Illuminate\Routing\Route
	 */
	protected function createRoute($methods, $uri, $action, $cacheTime) {
		...
		$route = $this->newRoute(
			$methods, $this->prefix($uri), $action, $cacheTime
		);
	 ...
	 * @return \Illuminate\Routing\Route
	 */
	protected function newRoute($methods, $uri, $action, $cacheTime) {
		return (new Route($methods, $uri, $action, $cacheTime))->setContainer($this->container);
	}
