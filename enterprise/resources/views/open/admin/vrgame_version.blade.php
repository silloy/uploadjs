...
<a href="javascript:history.back(-1)"><div class="ui basic  button ">返回游戏</div></a>
</div>
...
<table class="ui sortable celled table fixed">
  ...
  <tbody>
          ...
          @else
              ...
              <td class="right aligned" data-val="{{ $val["version_name"] }}" data-id="{{ $val["version_id"] }}">  @if($val["stat"]==0)<i  class="large  iconfont-fire iconfire-xianshangxianxia icon red action-sub"></i><i  class="large list layout icon  blue action-audit"></i><i  class="large minus circle icon red action-del"></i>@endif</td>
              </tr>
              @endforeach
          @endif
  </tbody>
...
$(function(){
  ...
  $(".action-audit").click(function() {
    ...
    permPost("/json/save/vrgame_subversion",{id:appid,version_name:versionName},function(res) {
      if(res.code==0) {
        var html = ''
        var checked = '';
...
function dataEdit(appid,versionName) {
  loi.edit("vrgame_version",{id:appid,version_name:versionName},function(data){
...
function chooseSubVersion() {
  ...
  permPost("/json/save/vrgame_version",{id:appid,version_name:versionName,version_id:version_id},function(res) {
    location.reload();
  });
}

function publishVersion() {
  permPost("/json/submit/vrgame_version",{id:appid,version_name:versionName},function(res){
    location.reload();
  })
}

function delVersion() {
  permPost("/json/del/vrgame_version",{id:appid,version_name:versionName},function(res){
    location.reload();
  })
}
</script>
