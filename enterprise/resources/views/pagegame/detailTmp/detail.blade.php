        ...
        <div class="fl banner cen">
            <div class="pr">
                ...
                @if(isset($images['screenshots']) && is_array($images['screenshots']) && $images['screenshots'])
                    ...
                    <li class="@if($jj==0) active @endif"><a><img src="{{$images['screenshots'][$jj]}}" /></a></li>
                    @endfor
                @endif
                ...
                @if(isset($news) && is_array($news) && $news)
                    @foreach($news as $index => $newsinfo)
                        @if(!isset($newsinfo['en']) || $newsinfo['en'] != "strategy")
                            <span class="f18 fl @if(isset($newsinfo['selected']) && $newsinfo['selected']) cur @endif" data-sortId="{{$newsinfo['id']}}" onclick="changeHref($(this).attr('data-sortId'))">{{$newsinfo['name']}}</span>
                        @endif
                    @endforeach
                @endif
                </p>
...
<script type="text/javascript">
    function changeHref(sortId) {
        var href = "//web.vronline.com/newslist/{{$appid}}/" + sortId;
