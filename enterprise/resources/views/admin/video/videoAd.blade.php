...
?>
                        ...
                        <div class="span12">
                            <!-- BEGIN SAMPLE FORMPORTLET-->
                                <div class="widget-body">
                                    <!-- BEGIN FORM-->
                                    <form class="form-horizontal" action="{{ url('uploadAd') }}" method="post" name="addForm" id="addForm" enctype="multipart/form-data">
                                        <div class="control-group">
                                            <label class="control-label">视频ID</label>
                                            <div class="controls">
                                                <input type="text" id="videoId" name="videoId" class="form-control input-large" placeholder="视频的ID号" aria-describedby="title" maxlength="32">
                                        ...
                                        <div class="control-group">
                                            ...
                                            <div class="controls">
                                                <input type="file" name="typelogo" id="typelogo" class="form-control input-large" aria-describedby="typelogo" maxlength="32" class="form-control"/>
                                                <span class="help-inline"></span>
                                            </div>
                                        </div>
                                        <div class="control-group">
    ...
    <ul class="nav nav-pills" role="tablist" id="sortList">
        @if(isset($sort) && count($sort)>0)
            @foreach($sort as $s)
                <li role="presentation" id="{{$s['vtid']}}"><a href="/videoAd/{{$s['vtid']}}">{{$s['typename']}}</a></li>
            @endforeach
        @endif
    </ul>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover text-center">
            <tr class="trHead">
                <th class="col-xs-1">{{ count($data) }}（视频编号）</th>
                <th class="col-xs-1">视频URL</th>
            ...
            @foreach ($data as $v)
                <tr>
                    <form class="form-inline" role="form" action="{{ url('uploadSort') }}" method="post" name="addForm" id="addForm{{ $v['content_id'] }}" enctype="multipart/form-data">
                    <td>
                    ...
                    <td>
                        ...
                        <?php
$url = url('/resources/404');
if ($v['content_id'] == 0 || $v['content_id'] == '') {
    $url = url('/resources/videoAd/' . substr(md5($v['content_url']), 0, 8) . '/' . substr(md5($v['content_url']), 0, 12));
} else {
    $url = url('/resources/videoAd/' . $v['content_id'] . '/' . substr(md5($v['content_id']), 0, 12));
}
?>
...
//            var userId = $(this).attr("data-userId");
        ...
        $("body").on("click", "a.sortDelBtn", function() {
            ...
            if (confirm("确认删除??")) {
                ...
                var ajaxUrl = "{{ url('video/videoAdDel') }}";
