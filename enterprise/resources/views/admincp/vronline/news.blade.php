...
{!! $blade->showHtmlClass('vronline_news_all',$curClass,'menu') !!}
</div>
<div class="ui small button right floated blue" onclick="dataEdit(0)"><i class="plus icon"></i>添加新闻</div>
...
<table class="ui sortable celled table fixed">
  ...
  <tbody>
          ...
          @else
              ...
              <td>{{  $blade->showHtmlClass('vronline_news',$val["article_category"]) }}</td>
...
function dataEdit(id) {
  location.href = "/vronline/newsEdit/"+id
}
...
$(".action-preview").click(function() {
    var id = $(this).attr("data-id");
    window.open("/vronline/newsPreview/"+id)
})
...
$(".action-search").keypress(function() {
  if(event.keyCode==13) {
     ...
     location.href = "/vronline/news?search="+searchText;
  }
});
...
$(".remove.link").click(function() {
    var searchText = $(this).prev().val();
    location.href = "/vronline/news";
})

function subData(tp) {
  if(tp==1) {
    if(sub_id>0) {
      permPost("/json/save/vronline_news_sub",{id:sub_id},function(res){
      location.reload();
    });
    }
  }
...
function deleteData(tp) {
  if(tp==1) {
    if(del_id>0) {
      permPost("/json/del/vronline_news",{del_id:del_id},function(data){
        location.reload();
      })
    }
  }
...
function audit(tp) {
  ...
  if(audit_id>0) {
    permPost("/json/pass/vronline_news",{edit_id:audit_id,tp:tp,msg:msg}, function(data){
      if(tp==1) {
