...
{!! $blade->showHtmlClass('3dbb_stat',$choose,'menu') !!}
</div>
...
<div class="ui icon input" style="width:400px">
  <input type="text" class="action-search" placeholder="搜索描述视频" value="{{ $searchText }}">
  @if($searchText)
...
<table class="ui sortable celled table fixed">
  ...
  <tbody>
          ...
          @else
              ...
              <td class="action-edit" data-val="{{ $val["source_url"] }}">{{ $val["name"] }}</td>
              <td>@if($val["source_tp"] == 1) 直传 @else 其他 @endif</td>
              <td>{{  $val["source_url"] }}</td>
              <td>@if($val["is_vr"] == 1) VR视频 @elseif($val["is_vr"] == 2) 3D视频 @endif</td>
              <td>{{ $val["intro"] }}</td>
              <td class="dev_uid">{!! $blade->showHtmlStat('3dbb', $val["stat"],'') !!}</td>
              <td class="right aligned">
                ...
                <i data-stat="{{ $val["stat"] }}" data-send="{{ $val["ctime"] }}" class="large  iconfont-fire iconfire-xianshangxianxia icon red action-del"></i> -->
              </td>
              </tr>
              @endforeach
          @endif
...
<div class="ui modal modal-audit">
  ...
  <div class="ui positive button" onclick="audit(3)">通过 </div>
  </div>
</div>
...
$(function(){
  ...
  if(event.keyCode==13) {
     ...
     location.href = "/vrhelp/dbb?search="+searchText;
  }
  });
...
function audit(tp) {
  ...
  if(audit_id>0) {
    permPost("/json/save/vrhelp_3dbb",{id:audit_id,tp:tp,msg:msg}, function(data){
      if(tp==3) {
...
function deleteData(tp) {
  if(tp==1) {
    if(del_id>0) {
      permPost("/json/del/vrhelp_3dbb",{del_id:del_id,del_tp:del_tp},function(data){
       location.reload();
      })
    }
  }
