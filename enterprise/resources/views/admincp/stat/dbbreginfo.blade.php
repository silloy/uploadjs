...
<table class="ui sortable celled table fixed">
  <thead>
    <tr>
      <th width="10%" title="UID">UID</th>
      <th width="10%" title="用户名">用户名</th>
      <th width="10%" title="手机号">手机号</th>
      <th width="10%" title="参与组别">参与组别</th>
      <th width="10%" title="拍摄设备">拍摄设备</th>
      <th width="10%" title="制作软件">制作软件</th>
      <th width="10%" title="注册时间">注册时间</th>
    </tr>
  </thead>
  <tbody>
...
<div>
  {!! $retDate->appends(['action'=> $action,'start' => $start,'end' => $end])->render() !!}
</div>
...
function loadData(dstart,dend) {
  location.href = 'http://admincp.vronline.com/stat/dbbreginfo?action=getselect&start=' +dstart+'&end='+dend;
}
</script>
@endsection
