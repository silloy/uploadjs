...
<div class="ui grid" style="clear:both">
  <div class="ten wide column">
        ...
        <div class="field">
            <label>分类</label>
            <select id="article_tp" class="ui dropdown"   >
            <option value="">请选择</option>
            {!! $blade->showHtmlClass('article','','select') !!}
            </select>
        </div>
        <div class="field">
  ...
  <div class="six wide column">
     <form class="ui form"  onsubmit="return false;">
        ...
        <div class="field">
            <label>来源链接</label>
            <input id="article_source_link" type="text">
        </div>
    </form>
  </div>
</div>
...
var article_tp = $('select.dropdown').dropdown();
var article_id = '{{ $id }}';
...
$(function () {
    ...
    loi.edit("news_article",article_id,function(data){
...
function save(style) {
    ...
    if(typeof(editData.err) == "undefined") {
        ...
        if(formData.article_content!=last_content || style==2) {
            permPost("/json/save/news_article",formData,function(res){
                if(res.code!=0) {
                    loiMsg(res.msg);
                } else {
