...
{!! $blade->showHtmlClass('article_all',$curClass,'menu') !!}
</div>
<div class="ui small button right floated blue" onclick="articleEdit(0)"><i class="plus icon"></i>添加新闻</div>
...
<table class="ui sortable celled table fixed">
  ...
  <tbody>
          ...
          @else
              ...
              <td class="action-preview" data-id="{{ $val["id"] }}">{{ $val["title"] }}</td>
              <td id="stat-{{ $val["id"] }}" data-val="{{ $val["stat"] }}">{{  $blade->showHtmlStat('article',$val["stat"]) }}</td>
              <td>{{  $blade->showHtmlClass('article',$val["tp"]) }}</td>
              <td>{{ $val["source"] }}</td>
              <td>{{ $val["author"] }}</td>
               <td>{{ $val["ctime"] }}</td>
              <td class="right aligned"><i class="large check circle icon teal action-audit"></i><i class="large send icon blue action-sub" data-id="{{ $val["id"] }}"></i>  <i class="large edit icon @if($val['stat']==1) grey @else blue @endif  action-edit" data-id="{{ $val["id"] }}"></i><i class="large minus circle icon red action-del"></i></td>
              </tr>
              @endforeach
          @endif
  </tbody>
...
function articleEdit(id) {
  location.href = "/news/articleEdit/"+id
}
...
$(".action-preview").click(function() {
    var id = $(this).attr("data-id");
    window.open("/news/articlePreview/"+id)
})
...
$(".action-search").keypress(function() {
  if(event.keyCode==13) {
     ...
     location.href = "/news/article?search="+searchText;
  }
});
...
$(".remove.link").click(function() {
    var searchText = $(this).prev().val();
    location.href = "/news/article";
})

function subData(tp) {
  if(tp==1) {
    if(sub_id>0) {
      permPost("/json/save/news_article_sub",{id:sub_id},function(res){
      location.reload();
    });
    }
  }
...
function deleteData(tp) {
  if(tp==1) {
    if(del_id>0) {
      permPost("/json/del/news_article",{del_id:del_id},function(data){
        location.reload();
      })
    }
  }
...
function audit(tp) {
  ...
  if(audit_id>0) {
    permPost("/json/pass/news_article",{edit_id:audit_id,tp:tp,msg:msg}, function(data){
      if(tp==1) {
