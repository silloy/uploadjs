...
class CallBackModel extends Model
{
    ...
    public function callBack($orderid, $paycenterid, $channel_order, $check_money, $addlock, &$addb_result = false)
    {
        ...
        if (!$orderid) {
            UdpLog::save2("pay.vronline.com/pay", array("function" => "callBack[model]", "result" => "false", "log" => "orderid miss", "orderid" => $orderid), __METHOD__ . "[" . __LINE__ . "]");
            $errlog = array("orderid" => $orderid, "action" => "callback", "errmsg" => "orderid错误", "tbl" => "order", "position" => __METHOD__ . "[" . __LINE__ . "]");
    ...
    public function callCommonDeveloper($appid, $consume)
    {
        ...
        if (!$serverid || $appinfo['use_default']) {
            if ($isdev) {
