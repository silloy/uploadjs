...
class OpenidModel extends Model
{
    ...
    public static function getOpenid($appid, $uid)
    {
        ...
        if(!$check || !is_array($check) || !isset($check['uid']) || !isset($check['appid']) || $check['appid'] != $appid || $check['uid'] != $uid) {
            UdpLog::save2("openid/fail", array("log" => "gen openid error", "appid" => $appid, "uid" => $uid, "openid" => $openid, "check" => $check), __METHOD__."[".__LINE__."]");
            return false;
        }
        return $openid;
    }
    ...
    private static function check()
    {
        ...
        if($count1 != $count2 || $count1 != self::$count || self::BASE_UID_DIVIDE !== 8385 || self::BASE_APPID_DIVIDE !== 9237) {
            UdpLog::save2("openid/fail", array("log" => "openidModel check false", "count1" => $count1, "count2" => $count2, "BASE_UID_DIVIDE" => self::BASE_UID_DIVIDE, "BASE_APPID_DIVIDE" => self::BASE_APPID_DIVIDE), __METHOD__."[".__LINE__."]");
            return false;
        }
        return true;
    }
