...
class Client {
	...
	var $soapinface = array('MongateSendSubmit', 'MongateGetDeliver', 'MongateGetDeliver', 'MongateQueryBalance', 'MongateGetDeliver', 'MongateMULTIXSend');
	var $defhandle = 0;
	var $arrret = array('string', 'string', 'string', 'int', 'string', 'string');
	/**
	 * 用户名
	 */
	var $userName;
	...
	function __construct($url, $type = 0, $timeout = 0, $response_timeout = 30) {
		$this->url = $url;
	...
	function SoapSendSms($arrParam) {
		...
		if ($this->isMode) {
			$response = $this->clt->__soapcall($this->soapinface[$this->defhandle], $this->GetSoapParam($arrParam));
		} else {
			$response = $this->clt->call($this->soapinface[$this->defhandle], $arrParam, $this->namespace);
		}
		return $response;
	}
	...
	function PostSendSms($arrParam) {
		...
		$statusCode = $this->clt->call($this->arrret[$this->defhandle], $arrParam);
		return $statusCode;
	}
	...
	function SoapComm($arrParam) {
		...
		if ($this->isMode) {
			$statusCode = $this->clt->__soapcall($this->soapinface[$this->defhandle], $this->GetSoapParam($arrParam));
		} else {
			$statusCode = $this->clt->call($this->soapinface[$this->defhandle], $arrParam, $this->namespace);
		}

		if (is_array($statusCode)) {
			return $statusCode[$this->arrret[$this->defhandle]];
